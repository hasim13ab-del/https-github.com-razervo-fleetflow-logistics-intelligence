...
export default function App() {
  ...
  // AUTH INITIALIZATION STATE: blocks ALL rendering until Firebase responds
  ...
  // ==========================================
  // LOADING GUARD — blocks render until auth state is known
  // ==========================================
  if (isLoading) {
    return (
  ...
  const triggerSimAlert = async (
    ...
    const newAlert: NotificationAlert = {
      id: 'alert_' + Date.now(),
      userId: riderId,
      riderName: riderObj?.fullName || '',
      employeeId: riderObj?.employeeId || '',
      triggerType,
      severity,
      message,
  ...
  const handleOFDScan = async (
    ...
    if (isRiderAbsentToday(riderId)) {
      return { success: false, message: 'Operation Denied: Rider marked Absent today.' };
    }
    const rider = users.find(u => u.uid === riderId);
    ...
    try {
      ...
      const newShipment: Shipment = {
        id: shipmentDocId, status: 'Assigned', timestamp: now, riderId,
        riderName: rider.fullName, hubId: rider.hubId,
        assignedBy: currentUser?.uid || '', assignedAt: now
      };
      ...
      await setDoc(doc(db, 'kpi', kpiId), {
        id: kpiId, riderId, date: today, hubId: rider.hubId,
        ofd: currentOfd + 1, delivered: existingKpi?.delivered || 0,
        failed: existingKpi?.failed || 0, pickup: existingKpi?.pickup || 0,
  ...
  const handleAddKPIEntry = async (newEntry: Omit<KPIEntry, 'id' | 'conversionRate'>) => {
    ...
    const finalTrackingId = newEntry.trackingId || '';
    ...
    try {
      await setDoc(doc(db, 'kpi', entryId), {
        ...newEntry, id: entryId, conversionRate: calculatedRate, trackingId: finalTrackingId
      });
    } catch (err) { console.error('Failed to write KPI entry:', err); }
  };
  ...
  const handleSetAttendance = async (userId: string, date: string, manualStatus: AttendanceStatus) => {
    ...
    } catch (err) { console.error('Failed to set attendance:', err); }
  };
  ...
  const handleUpdateHub = async (hubId: string, updatedFields: Partial<Hub>) => {
    try { await setDoc(doc(db, 'hubs', hubId), updatedFields, { merge: true }); }
    catch (err) { console.error('Failed to update hub:', err); }
  };
  const handleAddHub = async (newHub: Hub) => {
    try { await setDoc(doc(db, 'hubs', newHub.hubId), newHub); }
    catch (err) { console.error('Failed to add hub:', err); }
  };
  const handleDeleteHub = async (hubId: string) => {
    try { await deleteDoc(doc(db, 'hubs', hubId)); }
  ...
  const handleAddEmployee = async (newEmployee: Omit<User, 'uid'>) => {
    ...
    try { await setDoc(doc(db, 'users', newUid), { ...newEmployee, uid: newUid }); }
    catch (err) { console.error('Failed to add employee:', err); }
  };

  const handleFirebaseOnboarding = async (params: {
    fullName: string; email: string; role: Role; hubId: string; phoneNumber: string; employeeId: string;
    ...
    try {
      if (!auth.currentUser?.email) return { success: false, message: 'Admin session not found.' };
      ...
      const newUserProfile: User = {
        uid: newUid, fullName, organizationName: orgName, email, role, employeeId, hubId,
        status: 'active', phoneNumber,
        liveData: role === 'rider' ? {
          battery: 100, isIdle: false, gpsStatus: 'on' as const,
          lastActive: new Date().toISOString(),
          currentLocation: hubs.find(h => h.hubId === hubId)?.location || { latitude: 0, longitude: 0 }
        } : undefined
      };
      await setDoc(doc(db, 'users', newUid), newUserProfile);
      return { success: true, message: `User ${fullName} onboarded! Verification sent to ${email}. Password: ${generatedPassword}`, password: generatedPassword };
    } catch (err: any) {
  ...
  const handleReAuthSuperAdmin = async (password: string): Promise<boolean> => {
    try {
      ...
      return true;
    } catch (err) { console.error('Re-auth error:', err); return false; }
  };

  const handleUpdateStatus = async (uid: string, status: UserStatus) => {
    try { await setDoc(doc(db, 'users', uid), { status }, { merge: true }); }
    catch (err) { console.warn("Firestore sync status warning:", err); }
  };
  ...
  const handleClearAlert = async (alertId: string) => {
    try {
      await setDoc(doc(db, 'alerts', alertId), {
        resolved: true,
        actionTaken: 'Resolved by ' + (currentUser?.fullName || '')
      }, { merge: true });
  ...
  const handleCheckInMobile = async (userId: string, location: Location) => {
    ...
    try {
      await setDoc(doc(db, 'attendance', logId), {
        id: logId, userId, hubId: userObj?.hubId || '',
        date: today, checkIn: new Date().toISOString(),
        geofencePassed: true, locationAtCheckIn: location
      }, { merge: true });
  ...
  const handleStartDutyMobile = async (userId: string) => {
    const today = getTodayDate();
    const logId = `${userId}_${today}`;
    try { await setDoc(doc(db, 'attendance', logId), { startDuty: new Date().toISOString() }, { merge: true }); }
    catch (err) { console.error('Start duty failed:', err); }
  };
  ...
  const handleCheckOutMobile = async (userId: string) => {
    ...
    try {
      await setDoc(doc(db, 'attendance', logId), {
        endDuty: new Date().toISOString(), checkOut: new Date().toISOString(), totalHours: 0
      }, { merge: true });
  ...
  const handleReloadStatus = async () => {
    ...
    if (firebaseUser) {
      ...
      } catch (err) { console.error("Error reloading user status:", err); }
    }
  };
  ...
  return (
    <Routes>
      ...
      <Route path="/dashboard/:orgSlug" element={
      ...
      <Route path="/" element={
        <Navigate to={
          sessionStorage.getItem('orgSlug')
            ? `/dashboard/${sessionStorage.getItem('orgSlug')}`
            : '/login'
        } replace />
      } />
